...
namespace Unity.AI.Assistant.Editor.GraphGeneration
{
    ...
    {
        static string NormalizeId(string pathOrId)
        {
            return pathOrId
                .Replace("/", "_")
                .Replace("\\", "_")
        ...
        public static void RestructureGraph(DependencyGraph graph, string outputDir)
        {
            ...
            WriteNodeDir(outputDir, "nodes-scene", "scenes.json", sceneNodes);
            WriteNodeDir(outputDir, "nodes-asset", "assets.json", assetNodes);
            WriteNodeDir(outputDir, "nodes-tool", "tools.json", new List<object>());
            WriteNodeDir(outputDir, "nodes-assetType", "assetTypes.json", assetTypeNodes);
            WriteNodeDir(outputDir, "nodes-toolCategory", "toolCategories.json", new List<object>());
            WriteNodeDir(outputDir, "nodes-project", "project.json", new List<object> { projectNode });

            WriteNodeDir(outputDir, "edges-scene_dependsOn_asset", "dependencies.json", sceneAssetEdges);
            WriteNodeDir(outputDir, "edges-asset_dependsOn_asset", "dependencies.json", assetAssetEdges);
            WriteNodeDir(outputDir, "edges-asset_referencedBy_scene", "references.json", assetSceneEdges);
            WriteNodeDir(outputDir, "edges-assetType_include_asset", "type_membership.json", typeAssetEdges);
            WriteNodeDir(outputDir, "edges-toolCategory_include_tool", "category_membership.json", new List<object>());
            WriteNodeDir(outputDir, "edges-project_canUse_toolCategory", "canUse.json", new List<object>());
            WriteNodeDir(outputDir, "edges-project_has_scene", "has.json", projectSceneEdges);
            WriteNodeDir(outputDir, "edges-project_contains_assetType", "contains.json", projectAssetTypeEdges);
            WriteNodeDir(outputDir, "edges-asset_inheritsFrom_asset", "inheritance.json", inheritEdges);
            WriteNodeDir(outputDir, "edges-asset_implements_asset", "interface_implementation.json", implementEdges);
            WriteNodeDir(outputDir, "edges-asset_declares_asset", "field_declarations.json", declareEdges);
            if (usesEdges.Count > 0)
                WriteNodeDir(outputDir, "edges-asset_uses_asset", "type_usage.json", usesEdges);
